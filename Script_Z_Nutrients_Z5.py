...
for scenario in ['SSP1','SSP2','SSP3','SSP4','SSP5']:
  ...
  hydrology_monthly = nc.Dataset('/archive/depfg/graha010/duncan_copied_additional/hist-hydrology_monthly_variable_2011_2015.nc')
  discharge = hydrology_monthly['discharge']
  discharge = discharge[:,:,:]
  print(discharge.shape)
  discharge = discharge*1000000000000/12

  hybrid_tn = nc.Dataset('/archive/depfg/graha010/Analysis_Of_Nutrients/FUTURE_' + scenario + '_Hybrid_TN_2011_2015.nc')
  hybrid_tn = hybrid_tn['Hybrid_TN'][:,:,:]
  print(hybrid_tn.shape)


  monthly_load = hybrid_tn*discharge
  ...
  data = xarray.DataArray(corrected_concentration, coords=[('time', time), ('lat', lat), ('lon', lon)], name='HybTN_MB')
  
  data.attrs['name'] = 'Total Nitrogen (IMAGE-GNM with Random Forest and Mass Balance)'
  ...
  print('Saving')
  data.to_netcdf('/archive/depfg/graha010/Analysis_Of_Nutrients/FUTURE_' + scenario + '_Hybrid_TN_2011_2015_WithMB.nc')
  print('Saved')
