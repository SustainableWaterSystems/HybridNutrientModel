...
data_all = pd.read_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/Training_Nitro.csv',sep=';')
...
data_all = data_all.loc[data_all['Value'] < data_all['Value'].quantile(0.6)] 
print('Percentile process confirmed')


print(data_all['nitro'].min())
print(data_all['nitro'].max())
...
X = data_all[['q','tw','precip','nitro','Lat','Lon','Day','Station']]
y = data_all['Value']
...
joblib.dump(model,'/archive/depfg/graha010/Analysis_Of_Nutrients/ML_model_tn.joblib')
...
x_test = pd.DataFrame(x_test,columns=['q','tw','precip','nitro','Lat','Lon','Day','Station'])
x_test = x_test.reset_index(drop=True)

discharge = np.array(x_test['q'])
waterTemp = np.array(x_test['tw'])
precip = np.array(x_test['precip'])
nitro = np.array(x_test['nitro'])


print('Predicting started')
print(datetime.now())
predicted = model.predict(np.c_[discharge,waterTemp,precip,nitro])
print('Predicting finished')
print(datetime.now())

  
x_test['MonthlyTN'] = predicted
x_test.to_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/nitro_testing_sample.csv',index=False)


#x_train = pd.DataFrame(x_train,columns=['q','tw','precip','nitro','Lat','Lon','Day','Station'])
#x_train.to_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/nitro_training_sample.csv',index=False)


y_test = pd.DataFrame(y_test,columns=['Value'])
y_test.to_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/nitro_Y_testing_sample.csv',index=False)
