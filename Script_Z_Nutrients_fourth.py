...
## Monitoring data
observations_tn = pd.read_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/monthly_nitrogen.csv',sep=';')
observations_tn['YrMth'] = observations_tn['Day'].str[2:7]
data_lat = pd.DataFrame(observations_tn['Lat']).drop_duplicates().reset_index(drop=True)
data_lon = pd.DataFrame(observations_tn['Lon']).drop_duplicates().reset_index(drop=True)
...
observations_tn = observations_tn.merge(dates,how='left',on=['YrMth'])
observations_tn = observations_tn.dropna()
observations_tn = observations_tn.reset_index(drop=True)
observations_tn['YrMth_indexed'] = observations_tn['YrMth_indexed'].astype(int)
...
#print(observations_tn)
observations_tn = observations_tn.merge(data_lat,how='left',on=['Lat'])
observations_tn = observations_tn.merge(data_lon,how='left',on=['Lon'])
#print(observations_tn)
...
for jj in range(0,test_2.shape[0]):
  ...
  observations_MERGED = observations_tn.merge(extract_all,on=['Lat_converted','Lon_converted','YrMth_indexed'],how='left')
...
for ii in range(0,nitrogen.shape[0]):
  
  extract_nitro = nitrogen[ii,:,:]
  
  extract_lat = lat_copy
  extract_lon = lon_copy
  
  extract_nitro = np.reshape(extract_nitro,extract_nitro.shape[0]*extract_nitro.shape[1])
  extract_lat = np.reshape(extract_lat,extract_lat.shape[0]*extract_lat.shape[1])
  extract_lon = np.reshape(extract_lon,extract_lon.shape[0]*extract_lon.shape[1])
  
  extract_nitro = pd.DataFrame(extract_nitro,columns=['nitro'])
  extract_lat = pd.DataFrame(extract_lat,columns=['Lat_converted'])
  extract_lon = pd.DataFrame(extract_lon,columns=['Lon_converted'])
  
  extract_all = pd.concat([extract_nitro,extract_lat,extract_lon],axis=1)
...
y.to_csv('/archive/depfg/graha010/Analysis_Of_Nutrients/Training_Nitro.csv',sep=';',index=False)

print('saved: Updated for Nitrogen!')
